...
void SceneManager::update_scene(float deltaTime)
{
    ...
    if (glfwGetKey(renderer::get_instance().get_window(), GLFW_KEY_G)) {
        ...
        ContentManager::get_instance().post->passes.push_back(ContentManager::get_instance().grey);
        // Build post process
        content_manager::get_instance().build("display", ContentManager::get_instance().post);
    }
    if (glfwGetKey(renderer::get_instance().get_window(), GLFW_KEY_N)) {
        ContentManager::get_instance().post->passes.pop_back();
        ContentManager::get_instance().post->passes.push_back(ContentManager::get_instance().normal);
    }
    ...
    if (glfwGetKey(renderer::get_instance().get_window(), GLFW_KEY_P)) {
        ContentManager::get_instance().post->passes.pop_back();
        ContentManager::get_instance().post->passes.push_back(ContentManager::get_instance().pixelate);
        // Build post process
        content_manager::get_instance().build("display", ContentManager::get_instance().post);
    }
