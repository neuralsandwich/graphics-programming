...
void main() {
  ...
  vec4 diffuse = (mat.diffuse_reflection * light.colour) * max(dot(normal, light_dir), 0.0);

  // Calculate half_vector
  vec3 half_vector = normalize(light_dir + view_dir);

  //Calculate specular lighting
  vec4 specular = (mat.specular_reflection * light.colour) * pow(max(dot(normal, half_vector), 0.0), mat.shininess);
